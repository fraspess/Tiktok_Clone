...
function PaginationControls({hasNext, hasPrevious, currentPage, totalPages, onPageChange}: {hasNext: boolean; hasPrevious: boolean; currentPage: number; totalPages?: number; onPageChange: (page: number) => void}) {
    ...
    return <div className="flex items-center justify-between border-t border-border px-6 py-3">
...
function UsersTab() {
    ...
    return <div className="flex h-full min-h-0 flex-col"><div className="space-y-3 border-b border-border px-6 py-3"><div className="relative"><Search className="pointer-events-none absolute left-3 top-1/2 size-4 -translate-y-1/2 text-muted-foreground"/><Input value={search} onChange={(e) => { setSearch(e.target.value); setPage(1); }} placeholder={t("admin.searchUsers")} className="pl-9"/></div><StatusFilter value={filter} onChange={(value) => { setFilter(value); setPage(1); }}/></div><div className="min-h-0 flex-1 overflow-y-auto">
        {isLoading || isFetching ? <div className="flex justify-center py-8"><Loader2 className="animate-spin text-muted-foreground"/></div> : isError ? <p className="px-6 py-4 text-sm text-destructive">{t("admin.loadError")}</p> : users.length === 0 ? <p className="px-6 py-8 text-center text-sm text-muted-foreground">{search.trim() ? t("admin.noUsersFound") : t("admin.empty")}</p> : users.map((user) => <UserRow key={user.id} user={user}/>)}</div><PaginationControls hasNext={metadata?.hasNext ?? false} hasPrevious={metadata?.hasPrevious ?? false} currentPage={page} totalPages={metadata?.totalPages} onPageChange={setPage}/></div>;
}
...
function VideoRow({video}: {video: SimpleVideoDto}) {
    ...
    return <div className="flex items-center justify-between gap-3 border-b border-border px-6 py-3 last:border-b-0"><div className="flex min-w-0 items-center gap-3"><div className="relative h-14 w-10 shrink-0 overflow-hidden rounded-md bg-muted">{getMediaUrl(video.thumbnailUrl) && <img src={getMediaUrl(video.thumbnailUrl)} alt="" loading="lazy" className="absolute inset-0 h-full w-full object-cover" onError={(e) => e.currentTarget.remove()}/>}</div><div className="min-w-0"><p className="truncate text-sm font-medium">{description}</p><p className="truncate text-xs text-muted-foreground">@{getUsername(video.author?.username)}{hashtags.map((tag) => ` #${tag}`).join("")}</p><p className="text-xs text-muted-foreground">{t("admin.views", {count: video.viewCount ?? 0})}</p>{video.isBanned && <p className="text-xs text-destructive">{t("admin.banned")}</p>}</div></div>
...
function VideosTab() {
    ...
    return <div className="flex h-full min-h-0 flex-col"><div className="border-b border-border px-6 py-3"><StatusFilter value={filter} onChange={(value) => { setFilter(value); setPage(1); }}/></div><div className="min-h-0 flex-1 overflow-y-auto">{isLoading ? <div className="flex justify-center py-8"><Loader2 className="animate-spin text-muted-foreground"/></div> : isError ? <p className="px-6 py-4 text-sm text-destructive">{t("admin.loadError")}</p> : videos.length === 0 ? <p className="px-6 py-8 text-center text-sm text-muted-foreground">{t("admin.empty")}</p> : videos.map((video) => <VideoRow key={video.id} video={video}/>)}</div><PaginationControls hasNext={metadata?.hasNext ?? false} hasPrevious={metadata?.hasPrevious ?? false} currentPage={page} totalPages={metadata?.totalPages} onPageChange={setPage}/></div>;
}
...
function ReportsTab() {
    ...
    return <div className="flex h-full min-h-0 flex-col"><div className="border-b border-border px-6 py-3"><div className="flex flex-wrap gap-1">{REPORT_TYPES.map((type) => <Button key={type} variant={reportType === type ? "secondary" : "ghost"} size="sm" onClick={() => { setReportType(type); setPage(1); setResolved(new Set()); }}>{t(`admin.types.${type.toLowerCase()}`)}</Button>)}</div></div><div className="min-h-0 flex-1 overflow-y-auto">{isLoading ? <div className="flex justify-center py-8"><Loader2 className="animate-spin text-muted-foreground"/></div> : isError ? <p className="px-6 py-4 text-sm text-destructive">{t("admin.loadError")}</p> : reports.length === 0 ? <p className="px-6 py-8 text-center text-sm text-muted-foreground">{t("admin.empty")}</p> : reports.map((report) => { const content = report.reportedContent; const thumbnail = getAvatarUrl(content?.thumbnail ?? null); const isResolved = resolved.has(report.id); const date = report.createdAt ? new Date(report.createdAt) : null; const dateLabel = date && !Number.isNaN(date.getTime()) ? date.toLocaleString() : t("admin.unknownDate"); return <div key={report.id} className={cn("flex items-center justify-between gap-3 border-b border-border px-6 py-3 last:border-b-0", isResolved && "opacity-50")}><div className="min-w-0"><p className="text-sm font-medium">{t("admin.reportedBy")}: @{getUsername(report.reportedBy?.username)}</p>{content?.title && <p className="truncate text-xs text-muted-foreground">{content.title}</p>}{thumbnail && <img src={getMediaUrl(thumbnail)} alt="" loading="lazy" className="mt-1 h-10 w-10 rounded-md object-cover" onError={(e) => e.currentTarget.remove()}/>}<p className="text-xs text-muted-foreground">{t("admin.reason")}: {report.reason?.trim() || t("admin.unknownReason")} · {dateLabel}</p></div><div className="flex shrink-0 items-center gap-2">{!isResolved && reportType !== "Comment" && <BanReportAction report={report} reportType={reportType} onBanned={() => void resolve(report.id)}/>}<Button variant="outline" size="sm" disabled={isResolved || isResolving} onClick={() => void resolve(report.id)}>{isResolving && <Loader2 className="animate-spin"/>}{isResolved ? t("admin.processed") : t("admin.markProcessed")}</Button></div></div>; })}</div><PaginationControls hasNext={metadata?.hasNext ?? false} hasPrevious={metadata?.hasPrevious ?? false} currentPage={page} totalPages={metadata?.totalPages} onPageChange={setPage}/></div>;
}

function AdminPage() {
    const {t} = useTranslation();
    return <Card className="flex h-full min-h-0 flex-col"><CardHeader className="border-b border-border"><CardTitle>{t("admin.title")}</CardTitle></CardHeader><CardContent className="flex min-h-0 flex-1 flex-col p-0"><Tabs defaultValue="reports" className="flex min-h-0 flex-1 flex-col"><TabsList className="mx-6 mt-3 self-start"><TabsTrigger value="users">{t("admin.users")}</TabsTrigger><TabsTrigger value="videos">{t("admin.videos")}</TabsTrigger><TabsTrigger value="reports">{t("admin.reports")}</TabsTrigger></TabsList><TabsContent value="users" className="min-h-0 flex-1 overflow-hidden"><UsersTab/></TabsContent><TabsContent value="videos" className="min-h-0 flex-1 overflow-hidden"><VideosTab/></TabsContent><TabsContent value="reports" className="min-h-0 flex-1 overflow-hidden"><ReportsTab/></TabsContent></Tabs></CardContent></Card>;
}
