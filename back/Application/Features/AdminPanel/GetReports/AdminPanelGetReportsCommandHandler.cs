...
internal class AdminPanelGetReportsCommandHandler(IAppDbContext appDbContext, IStorageService storageService, UserManager<UserEntity> userManager) : IRequestHandler<AdminPanelGetReportsCommand, PagedResult<AdminReportDto>>
{
    public async Task<PagedResult<AdminReportDto>> Handle(AdminPanelGetReportsCommand request, CancellationToken cancellationToken)
    {
        ...
        return reports.MapItems(r => new AdminReportDto()
        {
            ...
            Status = r.Status,
            Reason = r.OtherReason ?? (r.Reason.HasValue ? r.ContentType switch
            {
